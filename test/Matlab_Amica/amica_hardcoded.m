function [A,c,LL,Lt,gm,alpha,mu,beta,rho] = amica_a(x,M,m,maxiter,update_rho,mindll,iterwin,do_newton,remove_mean,As,cs)
% function [A,c,LL,gm,alpha,mu,beta,rho] = amica_a(x,[M,m,maxiter,update_rho,mindll,iterwin,do_newton,As,cs])
%
% Perform AMICA algorithm for adaptive mixutre ICA on input data x (of dimension n x N), using M models,
...
if nargin >= 10
    ...
    plotfig = 1;  % can also set this to 1 to plot anyway (without the solution)
end
...
for h = 1:M %todo: wieder randomisieren
...
if m > 1
   mu = 0.1 * randn(m,n,M); %todo: zurück ändern
   
   mu(:,:,1) = [0.1 0.9; -0.01 0.0; 0.0 -0.02]
   mu(:,:,2) = [0.2 1; -0.01 0.0; 0.0 -0.03]
else
    mu = zeros(m,n,M);
end
beta = ones(m,n,M) + 0.1 * randn(m,n,M); %todo: zurück ändern
...
for iter = 1:maxiter
    ...
    for h = 1:M
        ...
        if M == 1
            b = pinv(A) * x; %source activity, A*b = x, Ahoch-1 *A * b = Ahoch-1 * x
        end
    ...
    for h = 1:M
        ...
        for i = 1:n
            for j = 1:m
                if M > 1
                    if m > 1
                        %z = wie gut ist jede der komponenten darin die
                        %daten zu beschreiben (gefittete probabilities)
                        %berechnet posterior probabilities aus z
                        z(i,:,j,h) = v(h,:) .* z(i,:,j,h);
                        sumz = sum(z(i,:,j,h));
