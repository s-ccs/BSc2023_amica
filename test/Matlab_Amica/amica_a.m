...
%added from amica10
switch 3
    case 1 %removes mean and sphere data
        xmn = mean(x,2);
        %S = pinv(sqrtm(cov(x')));
        for i = 1:n
            x(i,:) = x(i,:) - xmn(i);
        ...
        x = S*x;
    case 2 %only remove mean and normalize channels
        xmn = mean(x,2);
        for i = 1:n
            x(i,:) = x(i,:) - xmn(i);
        end
        S = diag( 1 ./ sqrt(var(x')) );
        x = S*x;
    otherwise %do not remove mean or sphere
        S = eye(n);
end
...
fix_inits = 1
switch fix_inits %0=random, 1=from file, 2=fixed value(a bad one)
    case 0 %randomizes A, beta and mu
        if m > 1
            mu = 0.1 * randn(m,n,M);
        else
            mu = zeros(m,n,M);
     ...
     case 1 %load from file
        %load("unit_test_data.mat", "mu_init")
        %load("breakit_data.mat", "mu_init")
        ...
        %load("breakit_data.mat", "beta_init")
...
for iter = 1:maxiter
    ...
    for h = 1:M
        switch 1
            ...
            case 2 %just to test different calc of Lt, dont use
                khinds(:,h) = ((h-1)*n+1):(h*n);
                W(:,:,h) = pinv(A(:,khinds(:,h)));
                ldet(h) = log(abs(det(W(:,:,h))));
                ldetS = log(abs(det(S)));
                Lt(h,:) = log(gm(h)) + ldet(h) +ldetS;
        end
