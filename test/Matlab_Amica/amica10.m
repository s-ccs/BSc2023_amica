...
fp = zeros(1,block_size);
Q = zeros(m,block_size);

if nargin >= 7 %todo: noch übersetzen
    Ltall = zeros(M,N);
end
...
for iter = 1:maxiter
    ...
    % identify components
    if mod(iter,ident_intvl) == 0 && share_comps == 1
        for h = 1:M
            for hh = (h+1):M
                for i = 1:n
                    for ii = 1:n
    ...
    if plotfig && mod(iter,dispinterval) == 0
        ...
        for h = 1:M
            for i = 1:n
                if nargin >= 7
                    plot([cs(1,h) cs(1,h)+As(1,i,h)],[cs(2,h) cs(2,h)+As(2,i,h)],'g','LineWidth',3);
                end
                plot([c(1,h) c(1,h)+A(1,khinds(i,h))],[c(2,h) c(2,h)+A(2,khinds(i,h))],'r','LineWidth',2);
            end
        end        
        axis([-Mx Mx -My My]);
        pause(0.5);
