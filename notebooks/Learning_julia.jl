...
# ╔═╡ 1b9e57c2-c70c-11ed-2964-9d24dac8ff6d
begin 
	using Distributions
	using StatsPlots
	using Pkg
...
# ╔═╡ 9cd36727-5e6d-4e72-96e4-607137e049a8
begin 
	n_g = 2
	N_g = 10000
	mu_g = [0, 0, 0]
...
# ╔═╡ 0fc018e9-a65c-4ce0-bc59-4da317eb0a76
begin
	    x = zeros(n_g,N_g)
	    for i in 1:n_g
...
# ╔═╡ 3fc898bb-731a-4c39-9e83-d0b0b45b17ac
begin 
	f = rand.(PGeneralizedGaussian.(mu_g, 1 ./beta_g, rho_g), N_g)
end


# ╔═╡ f3e86ea9-f820-4e88-be90-4cab31d9b7a7
MixtureModel(PGeneralizedGaussian.(mu_g, 1 ./beta_g, rho_g)...)

# ╔═╡ 8dc9dedd-3fe8-4938-af7b-5fa25c8e7ba0
typeof(f)

# ╔═╡ 699755da-54d5-4b89-acfc-c82a73be7a9c
 w = vcat(f...)

# ╔═╡ e7a6e0a9-e76d-4351-a3e1-c5880b594e76
#Test
v_Test = hcat(f...)

# ╔═╡ a2b92038-e1e5-41e7-9092-e3b7a895a323
#Test
md"""
hi
"""

# ╔═╡ 2d320767-6739-45b0-80d5-d5bff1775cde
density(x')

# ╔═╡ 880bb664-b989-4781-8754-724228ba48ea
begin
	#z = randn(10^3)
	y_test = rand(Gamma(1/1.5, 1), 10^3, 1)
	histogram(y_test)
end

# ╔═╡ fa39dfc4-5823-4436-884b-7ceca1aa587b
#Test
begin
	5*ones(1,5)
end

# ╔═╡ 085bf586-9d45-49e9-9cb3-7ecd42d6ca02
@__DIR__()

# ╔═╡ c5b3fa8f-bc01-4a40-aaa0-20876dc7c6d4
#Test
begin
	O = [1 2; 3 4; 5 6]
	for i in 1:2
...
# ╔═╡ 3a01e92e-285d-4b1d-b004-8efe90ba15b5
#beim aufruf von amica übergeben
begin
	#x
...
# ╔═╡ 1e0a04c9-c9fa-4e9f-b713-d66ebd3e0132
#im code gesetzt
begin
	fix_init = 1
	block_size = 3000
...
# ╔═╡ 893b2bb6-38e6-403a-9de3-fc4655ae4559
#Test
begin

	o = [1 2; 3 4]
	maximum(svd(o).S) #max != maximum
end

# ╔═╡ cda4b30f-bcea-4f8e-9df8-28b07684636c
...
begin
    ...
    for k in 1:num_comps
       #A[:,k] = A[:,k] / norm(A[:,k]); #norm different than in matlab?
    end
	gm = (1/M)*ones(m,n*M)
end

# ╔═╡ 8fbc92cf-ca51-40a2-9c42-e47a1c5a0934
#Sphering lines (110-134)
begin 
	if do_sphere == 1
...
# ╔═╡ 00626ed2-c0d3-44ed-9519-48cd4bc31a04
#Test
begin
	testm = [1 2; 3 6]
	decomp2 = svd(testm*testm'/2) #Us Ss Vs
    Ust = decomp2.U * diagm(1 ./sqrt.(decomp2.S)) * decomp2.U'

	var(testm', dims=1)
	S_test =  Diagonal(vec((1 ./ sqrt.(var(testm',dims=1)))))
end

# ╔═╡ 60caa3c2-a476-4f1c-a2b6-ac1d57e59ca7
#Test
log(abs(det([1 2; 3 4])))


# ╔═╡ fa61e438-1e2a-4032-8010-a80e853c87ec
#Test
begin
	h = [1 2; 3 4; 5 6]
	println(norm(h))
	println(sqrt(1+4+9+16+25+36))
end

# ╔═╡ ee3a83f9-e94c-4adb-9df4-6e84382d4048
begin
...
# ╔═╡ e03e482d-0f4b-4d27-9b8a-bb0710fcfebc
#allocate variables
begin
...
# ╔═╡ ba7a0e1a-0e40-49bb-a7ff-9266f09e7d16
#Test
density(v)

# ╔═╡ fa165890-2e01-4e8a-a609-1b7e7222692e
block_size

# ╔═╡ 77396336-9bbb-45d4-8258-91d3e6a7af80
#Lines 251 - ?
begin
	...
	for iter in 1:maxiter
		...
		blksize = block_size #warum ist das hier?
...
# ╔═╡ 3f8a8517-ddc5-47a8-820a-1ec26041ce14
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "559ca4921f80ba0936614e76a3ffd4e029146a82"
...
git-tree-sha1 = "436534fd2c533dd442543d32e29a517bd2b94a0b"
